                ...
                if len(det):
                    ...
                    for *xyxy, conf, cls in reversed(det):
                        ...
                        if save_img or save_crop or view_img:  # Add bbox to image
                            ...
                            #if abs(int(c1[0]) - int(c1[2]))/abs(int(c1[1]) - int(c1[3])) < 2/3 and abs(int(c1[0]) - int(c1[2]))/abs(int(c1[1]) - int(c1[3])) < 2/3
                            ...
                            try:
                                ...
                                1
                            all.append(str(names[int(c)]))
                            if str(names[int(c)]) not in output:
                                output.append(str(names[int(c)]))

                ui.printf('总数为：' + str(len(result)))
                label = str('count:' + str(len(result)))
                for st in output:
                    ui.printf(st + ' 数量为：' + str(all.count(str(st))))
                font = cv2.FONT_HERSHEY_SIMPLEX
                #im1 = cv2.putText(im1, label, (150, 150), font, 5, (0, 0, 255), 5)
...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        self.label.setStyleSheet("font-size:38px;font-weight:bold;font-family:SimHei;background:rgba(255,255,255,0);")
        ...
        self.pushButton_6.setStyleSheet("background:rgba(255,142,0,1);border-radius:10px;padding:2px 4px;")
    ...
    def retranslateUi(self, MainWindow):
        ...
        MainWindow.setWindowTitle(_translate("MainWindow", "YOLOv7绝缘子缺陷检测系统"))
        self.label.setText(_translate("MainWindow", "YOLOv7绝缘子缺陷检测系统"))
        ...
        self.pushButton_4.setText(_translate("MainWindow", "SSD算法检测"))
        self.pushButton_5.setText(_translate("MainWindow", "YOLOv7检测"))
        self.pushButton_6.setText(_translate("MainWindow", "YOLOR检测"))
